...
export default async function ShopPage({ params }: PageProps) {
  try {
    const { lang, slug } = await params;
    console.log("Rendering ShopPage for slug:", slug);

    // Find the shop matching the slug
    const shop = await fetchShopBySlug(slug);
    console.log("ShopPage: Found shop:", shop?.id, shop?.name);

    // Filter products by this shop
    const shopProducts = await fetchShopProducts(shop.id);
    console.log("ShopPage: Found products count:", shopProducts.length);
    ...
    console.error("CRITICAL SHOP PAGE ERROR:", error);
    throw error;
  }
}
