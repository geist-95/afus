...
export default function ShopClientWrapper({
  ...
  const [completedOrdersCount, setCompletedOrdersCount] = useState(shop.completed_orders_count || 0);

  useEffect(() => {
    async function loadStats() {
      try {
        const orders = await fetchOrders(shop.id);
        if (orders) {
          setCompletedOrdersCount(orders.length);
        }
      } catch (e) {
        console.warn("Could not load dynamic orders count:", e);
      }
    }
    loadStats();
  }, [shop.id]);
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const sessionUser = localStorage.getItem("afus_session_user");
      if (sessionUser) {
        try {
          const session = JSON.parse(sessionUser);
          if (session?.shop && session.shop.slug === slug) {
            setShop({
              ...session.shop,
              name: initialShop.name || session.shop.name,
              logo_url: initialShop.logo_url || session.shop.logo_url,
              banner_url: initialShop.banner_url || session.shop.banner_url,
              faq_translations: initialShop.faq_translations || session.shop.faq_translations || [],
              metadata: {
                ...session.shop.metadata,
                ...initialShop.metadata,
              }
            });
            if (session.shop.completed_orders_count !== undefined) {
              setCompletedOrdersCount(session.shop.completed_orders_count);
            }
            // Also sync owner info if available in session
            if (session.full_name || session.avatar_url) {
              setOwner({
                ...initialOwner,
                id: session.id,
                full_name: session.full_name,
                avatar_url: session.avatar_url || initialOwner?.avatar_url,
              });
            }
          }
        } catch (e) {
  ...
  const hasLogo = !!(shop.metadata?.logo_url || shop.logo_url);
  const logoUrl = hasLogo ? (shop.metadata?.logo_url || shop.logo_url) : '';
  const hasCover = !!(shop.metadata?.cover_url || shop.banner_url);
    ...
    <div className="w-full pb-12">
      ...
      <div className="w-full max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 mt-12">
        ...
        <div id="products" className="w-full">
          ...
          <div className="flex items-center gap-8 border-b border-neutral-200 pb-3 mb-6 overflow-x-auto scrollbar-none [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            ...
            <a href="#reviews" className="text-sm font-bold text-neutral-500 hover:text-black pb-3 -mb-[14px] whitespace-nowrap cursor-pointer transition-colors">
              {lang === 'fr' ? 'Avis' : lang === 'ar' ? 'التقييمات' : 'Reviews'} ({reviews.length})
            </a>
      ...
      <div className="w-full max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 mt-16 md:mt-24 space-y-16">
        ...
        {(shop.metadata?.announcement || shop.announcement) && (
          <section className="border-t border-neutral-200 pt-10 flex flex-col md:flex-row gap-8 lg:gap-12">
            <div className="w-full lg:w-[300px] xl:w-[320px] shrink-0">
              ...
              <p className="text-xs text-neutral-500">
                {lang === 'fr' ? 'Dernière mise à jour :' : 'Last updated:'} {shop.metadata?.announcement_updated_at || "May 03, 2021"}
              </p>
            </div>
            <div className="flex-1 w-full flex flex-col items-center md:items-start">
              <div className="w-full text-sm text-neutral-600 mb-4 whitespace-pre-wrap text-center md:text-left">
          ...
          <div className="w-full lg:w-[300px] xl:w-[320px] shrink-0">
            <h2 className="text-xl md:text-2xl font-bold text-black mb-4">{lang === 'fr' ? 'Avis' : lang === 'ar' ? 'التقييمات' : 'Reviews'}</h2>
             <div className="flex items-center gap-1 mb-2">
              <span className="font-bold text-lg text-black">{averageRating}</span>
              {averageRating !== "–" && (
        ...
        <section id="faq" className="border-t border-neutral-200 pt-10 pb-8 flex flex-col md:flex-row gap-8 lg:gap-12">
          ...
          <div className="flex-1 w-full space-y-4">
            {shop.faq_translations?.length > 0 ? shop.faq_translations.map((faq: any, idx: number) => (
              ...
              <div className="border border-neutral-200 rounded-xl p-5 text-center text-neutral-500 text-sm italic py-8">
                {lang === 'fr' ? 'Aucune FAQ pour le moment.' : lang === 'ar' ? 'لا توجد أسئلة شائعة حتى الآن.' : 'No FAQ yet.'}
              </div>
            )}
          </div>
        </section>
